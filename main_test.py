...
if IS_TEST:
    ...
    for idx_episode in range(n_episode_test):
        ...
        for test_step in range(n_step_per_episode):
            ...
            action_rand[:, :, 0] = np.random.randint(0, n_RB, [n_veh, n_neighbor])
            action_rand[:, :, 1] = np.random.randint(0, len(env.V2V_power_dB_List), [n_veh, n_neighbor])
            V2I_rate_rand, V2V_success_rand, V2V_rate_rand = env.act_for_testing_rand(action_rand)
            V2I_rate_per_episode_rand.append(np.sum(V2I_rate_rand))
            ...
            action_all_testing_sarl[i, j, 0] = action % n_RB
            action_all_testing_sarl[i, j, 1] = int(np.floor(action / n_RB))
            ...
            V2I_rate_per_episode_sarl.append(np.sum(V2I_rate_sarl))
                ...
                for j in range(n_RB):
                    for m in range(n_RB):
                        for n in range(n_RB):
                            ...
                            action_dpra[0, 0, 1] = int(np.floor(i / n_RB))

                            action_dpra[1, 0, 0] = j % n_RB
                            action_dpra[1, 0, 1] = int(np.floor(j / n_RB))

                            action_dpra[2, 0, 0] = m % n_RB
                            action_dpra[2, 0, 1] = int(np.floor(m / n_RB))

                            action_dpra[3, 0, 0] = n % n_RB
                            action_dpra[3, 0, 1] = int(np.floor(n / n_RB))
            ...
            action_testing_dpra[0, 0, 1] = int(np.floor(i / n_RB))

            action_testing_dpra[1, 0, 0] = j % n_RB
            action_testing_dpra[1, 0, 1] = int(np.floor(j / n_RB))

            action_testing_dpra[2, 0, 0] = m % n_RB
            action_testing_dpra[2, 0, 1] = int(np.floor(m / n_RB))

            action_testing_dpra[3, 0, 0] = n % n_RB
            action_testing_dpra[3, 0, 1] = int(np.floor(n / n_RB))
            ...
            V2I_rate_per_episode_dpra.append(np.sum(V2I_rate_dpra))
...
# 绘制V2I速率
axs[0].plot(range(len(V2I_rate_list)), V2I_rate_list, label='MARL')
axs[0].plot(range(len(V2I_rate_list_rand)), V2I_rate_list_rand, label='Random')
axs[0].plot(range(len(V2I_rate_list_sarl)), V2I_rate_list_sarl, label='SARL')
axs[0].plot(range(len(V2I_rate_list_dpra)), V2I_rate_list_dpra, label='DPRA')
axs[0].set_title('V2I Rate over Episodes')
axs[0].set_xlabel('Episode')
axs[0].set_ylabel('Mbps')
axs[0].legend()

# 绘制V2V成功率
axs[1].plot(range(len(V2V_success_list)), V2V_success_list, label='MARL')
axs[1].plot(range(len(V2V_success_list_rand)), V2V_success_list_rand, label='Random')
axs[1].plot(range(len(V2V_success_list_sarl)), V2V_success_list_sarl, label='SARL')
axs[1].plot(range(len(V2V_success_list_dpra)), V2V_success_list_dpra, label='DPRA')
axs[1].set_title('V2V Success Rate over Episodes')
